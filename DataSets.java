...
//______________________________________________________________________________//  
   ...
   public void fillRelHLowArr(DataPoint d)   
   {     
...
//______________________________________________________________________________//
   ...
   public void fillHLowArrFromDataSets(DataSets data) 
   {
      ...
      for (int i = 0; i < 1; i++) 
      {
         for (int j = 0; j < pos; j++) 
         {
            relHLowArrTemp[0][j] = (data.relHLowArr2[0][j]); 
 ...
 //______________________________________________________________________________// 
   public void fillRelHLowArrB(DataPoint data) 
   {
    ...
    for(int i = 0; i < pos; i++)  
    {
          relHLowArr2[0][i] = relHLowArr[0][i]; 
          relHLowArr2[1][i] = relHLowArr[1][i]; 
          relHLowArr2[2][i] = relHLowArr[2][i]; 
		         ...
		         {   
		            ...
		            pos--; 
		         	  	   ...
		         	  	   && data.getLow(relHLowArr[1][i]) >= data.getLow(relHLowArr[1][i+1])))
			         {
			        	 for(int j = i; j < pos; j++)
			             {
			                relHLowArr[0][j] = relHLowArr[0][j+1]; 
 ...
 //______________________________________________________________________________//
   public void magAndLen(DataPoint data)
   {
	   ...
	   lenReten = new double[pos+1];
		   ...
		   for(int i = 1; i < pos; i++)
		   {
			   magnitude[i] = Math.abs((data.getHigh(relHLowArr[0][i])) - (data.getLow(relHLowArr[0][i-1])));
			   i++;
		   }
		   for(int i = 2; i < pos; i++)
		   {
			   magnitude[i] = Math.abs((data.getLow(relHLowArr[0][i])) - (data.getHigh(relHLowArr[0][i-1])));
			   i++;
		   }
	   	}
	   	...
	   	for(int i = 1; i < pos; i++)
	   	{
	   		length[i] = (relHLowArr[2][i]) - (relHLowArr[2][i-1]); 
	   	}
 
		for(int i = 2; i < pos; i++)
		{
			magReten[i] = magnitude[i] / magnitude[i-1];
			lenReten[i] = length[i] / length[i-1];
		}	   
   }
